...
export default class Slider {
    constructor(settings) {
        ...
        if (
            (this.type === 'mobile' && windowWidth < 500) ||
            (this.type === 'desktop' && windowWidth > 500) ||
            !this.type
        ) {
            this.sliderInit();
        }
            ...
            if (this.flagX) {
                ...
                if (this.dir === 'prev') {
                    if (
                        ...
                        this.flag &&
                        ((this.infinity === false && cur + this.includeWindow !== this.items.length) ||
                            this.infinity === true)
                    ) {
    ...
    sliderInit() {
        ...
        if (this.calcHeight === true) {
            ...
            this.slider.style.height = `${this.getHeightCurrentStep(this.current)}px`;
        }
        ...
        if (this.infinity !== false) {
            this.addItems(
                'next',
                Math.ceil(this.right / this.widthItem) + Math.ceil(windowWidth / this.widthItem),
            );
            this.addItems(
                'prev',
                Math.ceil(this.left / this.widthItem) + Math.ceil(windowWidth / this.widthItem),
            );
        }
    ...
    moveLay(value, current, direction) {
        ...
        if (this.calcHeight === true) {
            ...
            this.slider.style.height = `${this.getHeightCurrentStep(
    ...
    moveAction(direction, speed) {
        ...
        let setPag = () => {
            ...
            this.items.forEach((item) => {
                ...
                if (this.pagenation && this.pagenation.dot) {
                    this.slider
                        .querySelector(`.${this.pagenation.dot}[data-id="${item.getAttribute('data-id')}"]`)
            ...
            document.dispatchEvent(
                new CustomEvent('change-slider', {
                    detail: { id: this.id, type: 'change', current },
                }),
            );
            if (this.pagenation) {
                if (this.pagenation.current) {
                ...
                if (
                    this.pagenation.dot &&
                    this.slider.querySelector(`.${this.pagenation.dot}[data-id="${current - 1}"]`)
                ) {
                    this.slider
        ...
        let getEl = (el) => {
            switch (direction) {
                case 'next':
                    ...
                    //     +el.nextElementSibling.getAttribute('data-id') + this.includeWindow > this.length
                    // ) {
                    //     return el;
                    // }
            ...
            case 'next':
                ...
                if (
                    (currentItem &&
                        ...
                        +currentItem.nextElementSibling.getAttribute('data-id') + this.includeWindow >=
                        this.length) ||
                    (currentItem && +currentItem.getAttribute('data-id') + this.includeWindow >= this.length)
                ) {
    ...
    setCurrentSlid(id) {
        if (this.infinity === false) {
            ...
            document.dispatchEvent(
                new CustomEvent('change-slider', {
                    detail: { id: this.id, type: 'change', current: +id },
                }),
            );
            if (this.buttons) {
                if (this.buttons.prev) {
